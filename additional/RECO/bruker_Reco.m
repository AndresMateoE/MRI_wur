...
%                         required parameters: RECO_rotate
%           'phase_corr_pi': when reconstructing phase images, most methods require a phase correction of pi per pixel
...
if sum(strcmp(recopart, 'all')) >=1
    recopart={'quadrature', 'phase_rotate', 'zero_filling', 'FT', 'phase_corr_pi', 'cutoff', 'sumOfSquares', 'transposition'};
end
    ...
    switch recopart{part}
        ...
        case 'phase_corr_pi'
            for NR=1:size(data,7)
                for NI=1:size(data,6)
                    for channel=1:size(data,5)
                        [ data(:,:,:,:,channel,NI,NR) ] = reco_phase_corr_pi( data(:,:,:,:,channel,NI,NR) , Reco, map(NI, NR) );
                    end
                end
            end            
